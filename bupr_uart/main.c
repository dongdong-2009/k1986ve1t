...
int main()
{
	...
	while(1)
	{
		//if((UART1->RIS & (1<<4)) ){					// check the buffer's threshold
		if( (UART1->RIS & (1<<6)) ){
...
void TimerConfig(void)
{
	...
	TIMER4->ARR = 10000 - 1;	// TIM4 period is 1 KHz  
	TIMER4->CCR1 = 5000;
...
void UartConfig(void)
{
	...
	UART1->IFLS |= (2 << UART_IFLS_RXIFLSEL_OFFS) | (2	 << UART_IFLS_TXIFLSEL_OFFS);  // threshold for FIFO is 7/8
	UART1->LCR_H |= UART_LCR_H_FEN;								// enable RX and TX FIFO
